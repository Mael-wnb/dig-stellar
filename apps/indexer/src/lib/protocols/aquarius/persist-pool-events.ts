// apps/indexer/src/lib/protocols/aquarius/persist-pool-events.ts
import type { Client, PoolClient } from 'pg';
import { AquariusNormalizedEventRow } from './types';
import {
...
export async function persistAquariusPoolEvents(params: {
  client: DbClient;
  entitySlug: string;
  expectedPoolId: string;
  rows: AquariusNormalizedEventRow[];
}) {
  ...
  for (const row of rows) {
    ...
    if (row.poolId !== expectedPoolId) {
      throw new Error(`Row poolId mismatch: ${row.poolId} !== ${expectedPoolId}`);
    }
  }
  ...
  for (const row of rows) {
    ...
    await client.query(
      ...
      [
        ...
        true,
        JSON.stringify({
          source: 'lib/protocols/aquarius/persist-pool-events',
          entitySlug,
