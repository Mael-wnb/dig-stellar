// apps/indexer/src/scripts/bootstrap/aquarius-upsert-core.ts
  ...
  try {
    ...
    const venueRes = await client.query(
      ...
      [
        ...
        JSON.stringify({
          source: 'aquarius_final_registry',
    ...
    const entityRes = await client.query(
      ...
      [
        ...
        JSON.stringify({
          ...
          token0: registry.pool.token0,
          token1: registry.pool.token1,
          info: registry.pool.info ?? null,
          source: 'aquarius_final_registry',
        }),
      ]
    );
    ...
    for (const asset of registry.assets ?? []) {
      const assetRes = await client.query(
        ...
        [
          ...
          asset.decimals,
          JSON.stringify({
            source: 'aquarius_active_pool_assets',
            generatedAt: registry.generatedAt,
          }),
        ]
      );
      ...
      let role = 'reserve';
      if (asset.contractId === registry.pool.token0) role = 'token0';
      if (asset.contractId === registry.pool.token1) role = 'token1';
    ...
    console.log({
      ...
      venueId,
      entityId,
      entitySlug: registry.pool.entitySlug,
      assetCount,
    });
  } catch (err) {
