...
async function main() {
  ...
  const reserveSnapshots = await loadJson<any>('61-aquarius-reserve-snapshots-db-ready.json');

  if (!poolSnapshot || !reserveSnapshots) {
    throw new Error('Missing Aquarius snapshot JSON files');
  }
  ...
  try {
    ...
    const entity = await getEntityBySlugOrThrow(client, poolSnapshot.entitySlug);
    ...
    for (const row of reserveSnapshots.rows ?? []) {
      ...
      await client.query(
        ...
        [
          ...
          row.estSupplyApy,
          JSON.stringify({
            source: '61-aquarius-reserve-snapshots-db-ready',
          }),
        ]
      );
    }
    ...
    console.log({
      completedAt: nowIso(),
      entitySlug: poolSnapshot.entitySlug,
      reserveCount: reserveSnapshots.rows?.length ?? 0,
    });
  } catch (err) {
