...
|---|---|---:|---|---|---|---|
| D1 — Multi-Wallet Portfolio & "Active Signer" Model | Partial | 82% | Medium | **Gap A done (criteria 1 + 3):** active-signer vs watch-only formalized — `user_wallets.is_active_signer` (DB-enforced singleton via `user_wallets_one_signer_per_user`), `PATCH /v1/wallets/:id/signer`, Kit-connect auto-promotes to signer, add-by-address stays watch-only, UI badges (Active Signer · connected/not-connected vs Watch-only) + two add-paths, signing guardrail in the swap widget. **Gap B part 1 done (criterion 2 — data layer):** Blend position resolver (`lib/protocols/blend/{fetch-user-positions,resolve-user-health}` + `81-stellar-wallet-blend-positions.ts`) writes per-asset supply/borrow → `wallet_protocol_positions` and a first-class `health_factor` per pool → new `wallet_pool_health` (HF from the SDK's `PositionsEstimate`, NULL = no debt). Runs for signer + watch-only, wired non-fatal into `refreshWallet`. Validated on mainnet (borrower HF 1.35/1.11 cross-checked = collateral/debt; no-position → zero rows). Plus prior: grouped multi-wallet, per-wallet balances, raw SQL v2 tables | **Gap B part 2 (criterion 2 — surfacing):** positions now persist but aren't exposed — needs `GET /v1/wallets/:id/positions` + a portfolio UI slice (supplied/borrowed/HF per wallet) to make the "aggregation" visible | Build Gap B part 2: API endpoint + UI portfolio slice reading `wallet_protocol_positions` + `wallet_pool_health` (closes D1 end-to-end; HF feeds D2's first alert family) |
